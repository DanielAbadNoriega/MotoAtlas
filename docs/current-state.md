...
- **Admin Models Studio**: Fases 1 a 6C.4 completadas (persistencia + image upload/storage) más UI polish (file input custom + Section Radar), post-publish navigation + App-level in-memory catalog sync y image replace/delete cleanup hardening cerrados. Siguientes fases recomendadas: multi-image gallery, A2 fields en draft si aplica, WebP conversion opcional, IntersectionObserver active section tracking.
