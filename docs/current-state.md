...
- Rama actual: `test/auth-fixtures-migration-batch-11-auth-provider`
- Último bloque validado: **auth fixtures migration batch 11** con Quality Gate aprobado. `src/features/auth/AuthProvider.test.tsx` ya usa factories centrales de auth desde `src/test/fixtures/auth.ts`, apoyándose en `createAuthUser`, `createSession` y `createUserProfile` para construir payloads repetidos sin puentear la lógica del provider.
- Alcance validado: `AuthProvider.test.tsx` quedó migrado como suite final y más sensible de esta iniciativa. `AuthProvider` sigue siendo la implementación bajo test; `createAuthState` NO se usó, no se inyectó ningún estado tipo `AuthContext` para bypass de comportamiento, y los mocks de Supabase/authService siguieron siendo el boundary conductual. Se preservaron loading inicial, no-session, sesión autenticada, resolución de perfil y cleanup de `onAuthStateChange`, sin tocar código productivo ni reinterpretar el gap P2 de profile/loading.
...
- `src/features/auth/AuthProvider.test.tsx` migrado en batch 11 a factories centrales de auth, preservando a `AuthProvider` como implementación bajo test y usando fixtures solo como data factories para `user/session/profile`. Aprendizaje validado: al cerrar esta suite final no se debía usar `createAuthState` ni ningún estado completo tipo `AuthContext` para bypass de comportamiento; los mocks de Supabase/authService siguieron marcando el boundary, y el gap P2 de profile/loading quedó documentado pero intacto.
...
- Backlog P2 Auth/testing (hardening): migración incremental de mocks `useAuth` ya cerrada sobre la base central de fixtures; el seguimiento restante pasa a smoke de staging/RLS, gap `ReviewModal` no-auth → RPC autenticada y gap transitorio de `profile/loading` en `onAuthStateChange`.
