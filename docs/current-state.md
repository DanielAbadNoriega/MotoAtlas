...
- **Suite de tests**: `1588` tests passing (83 files). Quality Gate vigente: `typecheck` clean + `git diff --check` clean.
...
### Workstream C — Admin gallery / AdminPage refactor (completado)

Último bloque validado: **AdminPage decomposition completa + gallery confirmed delete**. 1588 tests, typecheck clean, zero circular imports.

Branch: `refactor/admin-gallery-helpers`.
...
- **Confirmed immediate-delete**: botón `delete_forever` → `GalleryConfirmDeleteModal` → eliminación inmediata del gallery record + Storage cleanup best-effort con guard de path compartido. Cover fallback: si se elimina portada, se aplica placeholder automáticamente.
...
- Tests: 252 tests en focused `AdminPage.test.tsx` (confirmed-delete flow, Storage dedup, upload UX, gallery card polish, stable ordering, cover fallback, delete button visual, card back info fix).
...
- `AdminSidebar` import directo fijado en `AdminMotorcycleReviewsPage.tsx`.
...
### Workstream C — branch closure

La rama `refactor/admin-gallery-helpers` está completa desde el punto de vista de implementación:

- **AdminPage decomposition**: ~5900 → 13 líneas, 9 page components extraídos, `AdminModelFormBody` extraído (1462 líneas), `adminSharedUi` extraído, barrel aplanado, zero circular imports.
- **Gallery confirmed delete**: `GalleryConfirmDeleteModal`, eliminación inmediata al confirmar, Storage cleanup best-effort con guard de path compartido, cover fallback automático.
- **`AdminSidebar` import directo** fijado en `AdminMotorcycleReviewsPage.tsx`.
- **Quality Gate final de la rama**: 1588 tests / 83 files, typecheck clean, `git diff --check` clean.
- **No incluye** trabajo de landings ni capa SDD/spec/features.

La rama está lista para merge/sincronización con landings.
La capa SDD/spec/features se construirá después del merge, sobre el estado real combinado.

**Guardrails para futuras tareas:**
- No refactorizar, revertir, reorganizar ni tocar `src/components/pages/AdminPage/` salvo necesidad explícita y aprobada.
- No reintroducir imports desde páginas extraídas de vuelta a través de `./AdminPage` (barrel). Las páginas extraídas se importan desde su archivo directo; `AdminPage.tsx` es solo barrel de compatibilidad para `AdminPage.test.tsx`.
- Los helpers/constantes/hooks extraídos viven en `src/components/pages/AdminPage/admin*` y `useAdmin*`; no moverlos fuera de ese directorio.
- Esta rama no incluye landings/MotoIcon. No mezclar.

Backlog diferido (post-merge, no parte de esta rama):
- `App.tsx`: audit routing admin + `React.lazy()` para rutas admin.
- Architecture review: separación UI/hooks/services, feature boundaries.
- Quality Gate completo post-merge.
...
- **Global**: `1588` tests passing (83 files), typecheck clean, `git diff --check` clean.
- **AdminPage focused**: 252 tests (confirmed-delete, Storage dedup, delete button, card back info, cover fallback).
...
- Hardening `SECURITY DEFINER` aprobado: `public.is_admin()` y `public.handle_new_user_profile()` ya quedaron con revokes explícitos mínimos; `create_motorcycle_review_with_aspects(...)` se mantuvo sin cambios y con contrato auth-only cerrado.
...
- `src/components/pages/AccountMotorcycleReviewsPage/AccountMotorcycleReviewsPage.test.tsx` migrado en batch 5 a fixtures centrales, preservando el estado autenticado por defecto, el no-auth con `user/session/profile = null` e `isAuthenticated = false`, y el caso autenticado sin `user/profile` manteniendo la sesión presente. Con este cierre, la migración account-level queda completa.
- `src/components/pages/AdminMotorcycleReviewsPage/AdminMotorcycleReviewsPage.test.tsx` migrado en batch 6 a fixtures centrales, preservando la forma admin legacy por defecto y los overrides puntuales de no-auth / non-admin sin “mejorar” el shape por conveniencia. Aprendizaje validado: la migración account-level ya estaba cerrada y la migración admin empieza aquí; en suites admin hay que preservar `isAdmin`, `profile.role`, `user`, `session`, `isAuthenticated`, estados de loading/error y spies locales de auth sin que los defaults de fixture los sobrescriban.
- `src/components/pages/AdminPage/AdminPage.test.tsx` migrado en batch 7 a fixtures centrales, preservando la forma admin legacy por defecto y los overrides parciales más sensibles sin introducir `profile/session/admin role` por conveniencia. Aprendizaje validado: la migración admin ya incluye `AdminMotorcycleReviewsPage.test.tsx` y `AdminPage.test.tsx`; `AdminPage.test.tsx` es más amplia y exige preservar exactamente `user`, `session`, `profile`, `profile.role`, `isAuthenticated`, `isAdmin`, `isLoading`, estados de loading/error y spies locales, además de no tocar el área conocida de paginación/flaky salvo fallo directo.
- `src/components/pages/CommunityReviewsPage/CommunityReviewsPage.test.tsx` migrado en batch 8 a fixtures centrales, preservando el estado no-auth por defecto (`user/session/profile = null`, `isAuthenticated = false`, `isAdmin = false`, `isLoading = false`) y los overrides autenticados mínimos esperados por la suite. Aprendizaje validado: la migración community empieza aquí y debe mantener intactos los contratos de comunidad (`Útil N` pasivo en no-auth, ausencia de acciones falsas, ramas de own review / reported review / duplicate report, cleanup de report/reaction y orden editorial), sin que los defaults de fixture pisen spies locales.
- `src/components/pages/MotorcycleCommunityPage/MotorcycleCommunityPage.test.tsx` migrado en batch 9 a fixtures centrales, preservando el estado no-auth por defecto y la UX legacy propia de la página: acciones no-auth clicables con tooltip de login, hooks bloqueando antes de red y ramas de own review / reported review / duplicate report / cleanup / pending states sin cambios. Aprendizaje validado: no hay que forzar el comportamiento no-auth de `CommunityReviewsPage` sobre `MotorcycleCommunityPage`; los defaults de fixture tampoco deben pisar spies locales.
- `src/components/reviews/ReviewModal/ReviewModal.test.tsx` migrado en batch 10 a fixtures centrales, preservando el estado no-auth por defecto y los escenarios autenticados con `profile.displayName`. Aprendizaje validado posterior: el cleanup de `fix/review-modal-auth-contract` cerró el gap local UI/test; el modal ya no trata el submit no-auth como éxito soportado, mantiene guard defensivo con mensaje de login y preserva envío autenticado, validaciones, aspectos técnicos, accesibilidad, close/cancel y mocks de `createReviewWithAspects`.
- `src/features/auth/AuthProvider.test.tsx` migrado en batch 11 a factories centrales de auth, preservando a `AuthProvider` como implementación bajo test y usando fixtures solo como data factories para `user/session/profile`. Aprendizaje validado inicial: no se debía usar `createAuthState` ni ningún estado completo tipo `AuthContext` para bypass de comportamiento; los mocks de Supabase/authService siguieron marcando el boundary.
- Hardening posterior de `AuthProvider`: la transición de perfil durante `onAuthStateChange` ya quedó cerrada en `src/features/auth/AuthProvider.tsx` con `authTransitionRef`, loading explícito mientras `getUserProfile` resuelve y protección contra resoluciones async obsoletas. Resultado: el provider ya no expone el nuevo snapshot autenticado antes de que perfil/rol queden settleados, y `isAdmin` sigue derivando solo del `profile.role` resuelto.
- Adopción actual: 12 suites ya usan fixtures centrales (`AuthPage.test.tsx`, `StaticInfoPages.test.tsx`, `AccountRequestsPage.test.tsx`, `AccountPage.test.tsx`, `AccountReviewsPage.test.tsx`, `AccountMotorcycleReviewsPage.test.tsx`, `AdminMotorcycleReviewsPage.test.tsx`, `AdminPage.test.tsx`, `CommunityReviewsPage.test.tsx`, `MotorcycleCommunityPage.test.tsx`, `ReviewModal.test.tsx` y `AuthProvider.test.tsx`); la migración account-level quedó completa, la migración admin quedó cubierta en sus dos suites, la migración community quedó cubierta en sus dos suites, el modal ya quedó cubierto y la migración total de auth fixtures queda completa, sin áreas pendientes con mocks locales detectadas.
...
- Backlog P2/P3 Admin catálogo: `Admin Models Studio / Estudio de modelos` tiene **Fases 1 a 5C.1 + 6A-6C.4 + file input UI + Section Radar + post-publish navigation + catalog sync + image cleanup hardening + modal refactor + read-only gallery connection + gallery record creation + gallery card polish + stable ordering + cover fallback + confirmed immediate-delete (GalleryConfirmDeleteModal) + Storage cleanup best-effort + delete button visual + card back info fix implementadas**. Quedan pendientes: selección de primaria, reorden drag-and-drop, card back info simplificada, A2 fields en draft si aplica, WebP conversion opcional e IntersectionObserver active section tracking.
...
- **Admin Models Studio**: Fases 1 a 6C.4 + UI polish + post-publish navigation + catalog sync + image cleanup hardening + modal refactor + **read-only gallery connection + gallery record creation from uploads + gallery card polish + stable ordering + cover fallback + gallery primary sync fallback fetch + upload UX + stale galleryImagesRef fix + edit publish cleanup order fix + gallery-backed detection hardening + confirmed immediate-delete + GalleryConfirmDeleteModal + Storage cleanup best-effort** cerrados. **AdminPage decomposition completa** (~5900 → 13 líneas, barrel aplanado, `AdminSidebar` import directo fijado). Pendientes (galería): reorden drag-and-drop, descomposición JSX de galería+modal. Backlog diferido (post-merge): `React.lazy()` admin routes, architecture review.
...
- Fixtures de usuarios/perfiles para auth quedan como **P2 Auth baseline / Testing / Fixtures** con base central implementada y migración incremental cerrada. El trabajo restante asociado ya no es de fixtures: el smoke de staging sobre creación de reviews ya quedó validado y el hardening `SECURITY DEFINER` de funciones conocidas también quedó aprobado.
...
- `ReviewModal` ya endureció su contrato local: si se abre de forma inesperada sin sesión, el submit no llama `createReviewWithAspects` y muestra `Inicia sesión para escribir una review.`. El smoke desplegado ya confirmó compatibilidad con la RPC auth-only; la limitación no bloqueante restante es repetir el signup público directo cuando cese el rate limit 429 si se quiere esa evidencia exacta.
...
- La validación desplegada de creación de reviews ya quedó cerrada como smoke de comportamiento y el hardening `SECURITY DEFINER` de `is_admin()` / `handle_new_user_profile()` también quedó validado. Limitación abierta no bloqueante: repetir el signup público directo cuando cese el rate limit `429` si se quiere esa evidencia exacta.
