...
- Rama actual: `fix/auth-provider-profile-loading-transition`
- Último bloque validado: **AuthProvider profile loading hardening** con Quality Gate aprobado. `src/features/auth/AuthProvider.tsx` ya representa la resolución de perfil durante `onAuthStateChange` como loading, y `src/features/auth/AuthProvider.test.tsx` valida la transición con fixtures auth usados solo como data factories.
- Alcance validado: el hardening cerró el gap P2 donde `onAuthStateChange` podía exponer un estado autenticado aparentemente settleado con `profile=null` / `isAdmin=false`. `AuthProvider` sigue siendo la implementación bajo test; `createAuthState` NO se usó, los mocks de Supabase/authService siguen siendo el boundary conductual y `authTransitionRef` evita que resoluciones async viejas sobrescriban el estado auth más nuevo. Se preservaron bootstrap inicial, no-session, sesión autenticada, derivación admin y cleanup de `onAuthStateChange`.
- Tests: 1144 passed (74 files)
- Typecheck: clean
...
- `src/components/reviews/ReviewModal/ReviewModal.test.tsx` migrado en batch 10 a fixtures centrales, preservando el estado no-auth por defecto y los escenarios autenticados con `profile.displayName`, sin reinterpretar el gap conocido entre el modal no-auth y la RPC autenticada. Aprendizaje validado: `ReviewModal` es sensible por formar parte del flujo de creación de reviews; la migración no debe cambiar envío autenticado, validaciones, aspectos técnicos, accesibilidad, close/cancel ni mocks de `createReviewWithAspects`, y los defaults de fixture tampoco deben pisar spies locales.
...
- Backlog P2 Auth/testing (hardening): migración incremental de mocks `useAuth` ya cerrada sobre la base central de fixtures; el seguimiento restante pasa a smoke de staging/RLS y validación/derivación server-side del alias visible en reviews autenticadas, además del gap `ReviewModal` no-auth → RPC autenticada.
...
- Fixtures de usuarios/perfiles para auth quedan como **P2 Auth baseline / Testing / Fixtures** con base central implementada y migración incremental cerrada. El trabajo restante asociado ya no es de fixtures, sino de hardening: smoke de staging/RLS, gap `ReviewModal` no-auth → RPC autenticada y validación/derivación server-side del alias visible en reviews autenticadas.
...
- Hardening de `AuthProvider` ya aplicado: `onAuthStateChange` vuelve a representar la resolución de perfil como loading y evita exponer como settleado un estado autenticado sin rol resuelto. El riesgo auth restante pasa por smoke de staging/RLS, gap `ReviewModal` no-auth → RPC autenticada y validación/derivación server-side de `p_user_name`.
