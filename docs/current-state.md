...
- Rama actual: `feature/page-hero-community-base`
- Último bloque validado: **Fase A de la unificación de heroes** (rama `feature/page-hero-community-base`). Safe Builder introdujo la base normalizada `PageHero` (`src/components/ui/PageHero/PageHero.tsx` + `PageHero.scss` + `index.ts`) con API mínima: `titleId`, `title`, `eyebrow?`, `description?`, `imageSrc?`, `imageAlt?`, `className?`, `children?`, `actions?`. Migradas las 4 páginas admin (Dashboard, Reviews, Requests, Moderation) a `PageHero` sin cambio visual. Migrada `CommunityRankingsPage` a `PageHero` quitando las acciones del hero (`Explorar rankings`, `Ver comunidad`) porque la navegación vivirá en una futura navbar/subnav. Migrada `TopRatedMotorcyclesPage` con nueva prop opcional `variant?: 'community' | 'topRated'`: en `#/comunidad` (`variant="community"`) el hero queda sin acciones; en `#/motos-mejor-valoradas` (`variant="topRated"`) el hero mantiene `Explorar comunidades` y `Comparar motos`. `CommunityHero` reconvertido en thin wrapper `@deprecated` alrededor de `PageHero` para mantener compatibilidad con importadores externos. SCSS admin ajustado: `.community-hero__content` → `.page-hero__content`; la regla `display: none` para `.community-hero__actions` se elimina porque `PageHero` admin no recibe `actions`. `CommunityReviewsPage` (hero local con doble gradient) **no se migra en esta fase** — queda pendiente para Fase B. `#/comunidad/reviews` conserva su hero local intacto. Sin cambios de schema/RLS/auth/routes.
...
- La rama `feature/page-hero-community-base` queda registrada como **Fase A de la unificación de heroes/CTAs**: base normalizada `PageHero` (Fase A), `CommunityHero` reusado como wrapper deprecated. Las acciones del hero de `#/comunidad` y `#/comunidad/rankings` se removieron porque la navegación vivirá en una futura navbar/subnav. La Fase B queda como backlog para migrar `CommunityReviewsPage` (hero local con doble gradient) y, eventualmente, los heroes de `BikeDetailPage`, `MotorcycleCommunityPage`, `AccountPage` y las páginas estáticas. La fase 13.1 de `product-roadmap.md` sigue siendo la referencia para HeroAction system, Button variants y mobile-first.
- ...
