...
- Rama actual: `feature/admin-models-image-gallery-audit`
- Último bloque validado: **Admin Models read-only motorcycle image gallery modal connection** aprobado.
- Alcance validado:
  ...
  - **Read-only gallery modal connection**: el image manager modal en edit mode carga imágenes reales desde `getAdminMotorcycleGalleryImages` con estados de carga, error, vacío y grid de galería. Reemplaza el placeholder "Gestor visual en preparación" por la galería real de solo lectura. La nota del modal se actualizó a "La galería es de solo lectura. La gestión completa (subir, editar, reordenar) se añadirá en una fase posterior."
- Tests: 1349 passed (78 files)
- Typecheck: clean
- `git diff --check`: clean
- Focused checks más recientes:
  - focused gallery modal connection (AdminPage) → `5 tests` passing (read-only gallery fetch, loading, error, empty, gallery grid)
  - focused schema + gallery service foundation → `2 files / 102 tests` passing
  - suite completa → `1349` tests passing
- La galería multiimagen ya tiene base de schema/RLS + service layer + conexión de lectura en el modal admin. Quedan pendientes la escritura y gestión completa (creación de records desde uploads, selección de primaria, reorden, borrado coordinado records/Storage), WebP conversion opcional, IntersectionObserver active section tracking y A2 fields en draft si aplica.
...
- Backlog P2/P3 Admin catálogo: `Admin Models Studio / Estudio de modelos` tiene **Fases 1 a 5C.1 + 6A-6C.4 + file input UI + Section Radar + post-publish navigation + catalog sync + image cleanup hardening + modal refactor + read-only gallery connection implementadas**. Quedan pendientes: gestión completa de galería (creación de records desde uploads, selección de primaria, reorden, borrado coordinado), A2 fields en draft si aplica, WebP conversion opcional e IntersectionObserver active section tracking.
...
- **Admin Models Studio**: Fases 1 a 6C.4 + UI polish + post-publish navigation + catalog sync + image cleanup hardening + modal refactor + **read-only gallery connection** cerrados. Siguientes fases recomendadas: completar la galería con escritura (creación de records desde uploads, selección de primaria, reorden, borrado coordinado), A2 fields en draft si aplica, WebP conversion opcional e IntersectionObserver active section tracking.
