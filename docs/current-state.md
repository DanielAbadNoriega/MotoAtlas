...
- Rama actual: `test/auth-fixtures-migration-batch-10-review-modal`
- Último bloque validado: **auth fixtures migration batch 10** con Quality Gate aprobado. `src/components/reviews/ReviewModal/ReviewModal.test.tsx` ya usa fixtures auth centrales desde `src/test/fixtures/auth.ts`, manteniendo el boundary mockeado de `useAuth` mientras crea el estado con `createAuthState`, `createAuthUser`, `createSession`, `createUserProfile` y `mockUnauthenticatedAuthState`.
- Alcance validado: se preservó el shape legacy no-auth por defecto de `ReviewModal.test.tsx` y sus overrides autenticados con `profile.displayName`, sin introducir `profile/session/admin role` por conveniencia. Aprendizaje validado: `ReviewModal` sigue siendo sensible por formar parte del flujo de creación de reviews; la migración NO cambió el comportamiento de producto, NO reinterpretó el gap conocido entre no-auth y RPC autenticada, y mantuvo intactos envío autenticado, validaciones, aspectos técnicos, accesibilidad del modal, close/cancel y mocks de `createReviewWithAspects`, además de evitar que los defaults de fixture pisen spies locales (`signIn`, `signUp`, `signOut`, `refreshProfile`).
...
- Adopción actual: 11 suites ya usan fixtures centrales (`AuthPage.test.tsx`, `StaticInfoPages.test.tsx`, `AccountRequestsPage.test.tsx`, `AccountPage.test.tsx`, `AccountReviewsPage.test.tsx`, `AccountMotorcycleReviewsPage.test.tsx`, `AdminMotorcycleReviewsPage.test.tsx`, `AdminPage.test.tsx`, `CommunityReviewsPage.test.tsx`, `MotorcycleCommunityPage.test.tsx` y `ReviewModal.test.tsx`); la migración account-level queda completa, la migración admin ya incluye sus dos primeras suites, la migración community ya incluye sus dos suites iniciales, la migración del modal ya quedó cubierta y queda 1 área con mocks locales detectada: `AuthProvider.test.tsx`.
