...
- Rama actual: `feature/auth-fixtures-migration-batch-5`
- Último bloque validado: **auth fixtures migration batch 5** con Quality Gate aprobado. `src/components/pages/AccountMotorcycleReviewsPage/AccountMotorcycleReviewsPage.test.tsx` ya usa fixtures auth centrales desde `src/test/fixtures/auth.ts`, manteniendo el boundary mockeado de `useAuth` mientras crea el estado con `createAuthState`, `createAuthUser`, `createSession`, `createUserProfile` y `mockAuthenticatedAuthState`.
- Alcance validado: se preservaron el estado autenticado por defecto, el escenario no-auth con `user/session/profile = null` e `isAuthenticated = false`, y el caso autenticado sin `user/profile` manteniendo la sesión presente. Aprendizaje validado: en suites account-level, la forma legacy autenticada por defecto también es contrato y debe preservarse; además, los defaults de fixture no deben pisar mocks locales de función como `signOutMock`. La migración account-level queda completa con este batch. No se tocaron product code, `AuthProvider`, schema/RLS/Supabase policies ni docs fuera del bloque de auth/testing.
...
- Adopción actual: 6 suites ya usan fixtures centrales (`AuthPage.test.tsx`, `StaticInfoPages.test.tsx`, `AccountRequestsPage.test.tsx`, `AccountPage.test.tsx`, `AccountReviewsPage.test.tsx` y `AccountMotorcycleReviewsPage.test.tsx`); la migración account-level queda completa y quedan 6 áreas con mocks locales detectadas: `AdminMotorcycleReviewsPage.test.tsx`, `AdminPage.test.tsx`, `CommunityReviewsPage.test.tsx`, `MotorcycleCommunityPage.test.tsx`, `ReviewModal.test.tsx` y `AuthProvider.test.tsx`.
