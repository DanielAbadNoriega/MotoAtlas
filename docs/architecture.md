...
- **Galería de solo lectura conectada al modal**: al abrir el modal en edit mode se cargan imágenes desde `getAdminMotorcycleGalleryImages` con estados de carga, error, vacío y grid de galería. No hay datos falsos ni mock gallery cards.
...
- `motorcycle_images` no sustituye ese contrato: añade una capa paralela de metadata de galería que el modal admin ya consume en modo solo lectura.
