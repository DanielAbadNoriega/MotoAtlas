...
Debajo del hero hay un bloque editorial separado del garaje filtrable: `Reviews destacadas`, `Últimos reportes` e `Insights en vivo` se calculan desde reviews `approved` cargadas y no dependen de los filtros. Los filtros solo afectan a `Garaje de la comunidad`, que agrupa por `motorcycleId` y calcula rating medio, número de reviews, última review, uso más repetido y kilómetros declarados. El panel de filtros es apply-on-change en tiempo real; el botón "Aplicar" cierra el panel en mobile. El aside `Insights en vivo` usa un panel técnico compacto inspirado en `comunidad-reviews.html`, con iconos Material Symbols y datos reales calculados. **Pendiente: refresco suave de insights en vivo.**
